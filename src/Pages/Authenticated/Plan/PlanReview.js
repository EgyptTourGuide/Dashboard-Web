...
const PlanReview = (props)=>{
    ...
    useEffect(()=>{
        async function getInfo(){
            try{
                let Q_res = await authAxios.get(`${URL}/plans/${props.match.params.id}/form`)
                let C_res = await authAxios.get(`${URL}/plans/${props.match.params.id}`)
                let R_res = await authAxios.get(`${URL}/plans/${props.match.params.id}/review/requests`)
                setplanForm(Q_res.data.hotelForm)
                setComments(C_res.data.plan.reviews)
    ...
    const accept = async(id)=>{
        ...
        const res = await authAxios.post(`${URL}/plans/${planForm.id}/review/requests`, body)
        if(res.status === 200){
    ...
    return(
        <Layout to={`/plans/${planForm.id}`} head={planForm !== null && `Plans / ${planForm.name} / Reviews`}>
             <div style={{display: 'flex', justifyContent: 'center', alignItems: 'center', flexDirection: 'column', marginTop: 15}}>
                    ...
                    { data === 'comments' ? comments.length > 0 && comments.map(review=><ReviewCard comment key={review.id} review={review}/>) : (
                        reviews.length > 0 && reviews.map(review=><ReviewCard accept={accept} decline={decline} conditions={planForm.conditions} key={review.id} review={review}/>))
                    }
             </div>
        </Layout>
    )
