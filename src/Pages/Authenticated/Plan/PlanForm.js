...
const PlanForm = (props)=>{
    ...
    const [ planForm, setForm ] = useState(null) 
    ...
    useEffect(()=>{
            async function getInfo(){
                try{
                    let res = await authAxios.get(`${URL}/plans/${props.match.params.id}/form`)
    ...
    const onAdd = ()=>{
        const q = planForm.questions
        if(q.length < 4 && text){
            setForm({...planForm, questions: [...q, text]})
            setText('')
        }
    }
    ...
    const onDelete = (value)=>{
        ...
        setForm({...planForm, questions: temp})
    }
    ...
    const onSave = async()=>{
           ...
           console.log(first)
           console.log(second)
           console.log(planForm.questions)

           if(first && second && planForm.questions.length === 4){

                const newForm = {
                    questions: planForm.questions,
                    conditions: JSON.stringify([first, second])
                }

               const req = await authAxios.post(`${URL}/plans/${planForm.id}/form`, {planForm: {...newForm}})
               if(req.status === 200){
                setMsg('Saved')
               }
           }else{
    ...
    return(
        <Layout to={{pathname: `/plans/${props.match.params.id}`}} head={`${planForm !== null && `Plans / ${planForm.name} / Form`}`}>
            ...
            <div style={styles.qCon}>
                ...
                {planForm && <ol>
                     {planForm.questions.map(q=><li key={q}><div style={{width: 100}}><ListWithDelete value={q} key={q} onDelete={onDelete} /></div></li>)}
                 </ol>}
                 </div>
