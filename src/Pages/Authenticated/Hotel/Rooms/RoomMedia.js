   ...
   const save = async()=>{
        ...
        if(files.length > 0){
            ...
            for(let i = 0; i < files.length; i++){
             ...
             res = await authAxios.put(`${URL}/hotels/room/${props.match.params.roomId}`, fd ,options)
        }else{
            let d = data.media
             res = await authAxios.put(`${URL}/hotels/room/${props.match.params.roomId}`, {old: d} ,options)
        }   
         setLoading(false)
         if(res.status === 200){
            setMessage('Saved')
    ...
    const onDelete = async(index)=>{
        ...
        d.splice(index, 1)
        try{
           let res = await authAxios.put(`${URL}/hotels/room/${props.match.params.roomId}`, {old: d} ,options)
           if(res.status === 200)
            setData({...data, media: res.data.media})
        }catch(e){
           console.log(e)
    ...
    return(
       <Layout to={{pathname: `/hotels/${props.match.params.id}/rooms/${props.match.params.roomId}`}} head={`${data.name} / Rooms / ${room.number} / Media`}>
           <div style={{
                marginTop: 12,
                display: 'flex',
