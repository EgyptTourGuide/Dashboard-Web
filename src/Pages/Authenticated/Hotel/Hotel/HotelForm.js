...
const HotelForm = (props)=>{
    ...
    const [ hotelForm, setForm ] = useState(null) 
    ...
    useEffect(()=>{
            async function getInfo(){
                try{
                    let res = await authAxios.get(`${URL}/hotels/${props.match.params.id}/form`)
                    const { questions, conditions, name, id } = res.data.hotelForm
    ...
    const onAdd = ()=>{
        const q = hotelForm.questions
        if(q.length < 4 && text){
            setForm({...hotelForm, questions: [...q, text]})
            setText('')
        }
    }
    ...
    const onDelete = (value)=>{
        ...
        setForm({...hotelForm, questions: temp})
    }
    ...
    const onSave = async()=>{
           ...
           console.log(first)
           console.log(second)
           console.log(hotelForm.questions)

           if(first && second && hotelForm.questions.length === 4){

                const newForm = {
                    questions: hotelForm.questions,
                    conditions: JSON.stringify([first, second])
                }

               const req = await authAxios.post(`${URL}/hotels/${hotelForm.id}/form`, {hotelForm: {...newForm}})
               if(req.status === 200){
                setMsg('Saved')
               }
           }else{
    ...
    return(
        <Layout to={{pathname: `/Hotels/${props.match.params.id}`}} head={`${hotelForm !== null && `Hotels / ${hotelForm.name} / Form`}`}>
            ...
            <div style={styles.qCon}>
                ...
                {hotelForm && <ol>
                     {hotelForm.questions.map(q=><li key={q}><div style={{width: 100}}><ListWithDelete value={q} key={q} onDelete={onDelete} /></div></li>)}
                 </ol>}
                 </div>
