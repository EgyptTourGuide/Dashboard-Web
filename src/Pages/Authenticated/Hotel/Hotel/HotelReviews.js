...
const HotelReview = (props)=>{
    ...
    useEffect(()=>{
        async function getInfo(){
            try{
                let Q_res = await authAxios.get(`${URL}/hotels/${props.match.params.id}/form`)
                let C_res = await authAxios.get(`${URL}/hotels/${props.match.params.id}`)
                let R_res = await authAxios.get(`${URL}/hotels/${props.match.params.id}/review/requests`)
                sethotelForm(Q_res.data.hotelForm)
                setComments(C_res.data.reviews)
    ...
    const accept = async(id)=>{
        ...
        const res = await authAxios.post(`${URL}/hotels/${hotelForm.id}/review/requests`, body)
        if(res.status === 200){
    ...
    return(
        <Layout to={`/Hotels/${hotelForm.id}`} head={hotelForm !== null && `Hotels / ${hotelForm.name} / Reviews`}>
             <div style={{display: 'flex', justifyContent: 'center', alignItems: 'center', flexDirection: 'column', marginTop: 15}}>
                    ...
                    { data === 'comments' ? comments.length > 0 && comments.map(review=><ReviewCard comment key={review.id} review={review}/>) : (
                        reviews.length > 0 && reviews.map(review=><ReviewCard accept={accept} decline={decline} conditions={hotelForm.conditions} key={review.id} review={review}/>))
                    }
             </div>
        </Layout>
    )
